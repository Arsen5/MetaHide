...
public class Model : ISteganography
{
    ...
    public (bool success, string message, string outputPath) HideData(string imagePath, string data)
    {
        try
        {
            ...
            // 1. Удаляем старый EXIF (если есть)
            byte[] cleanedBytes = RemoveExifSegment(File.ReadAllBytes(imagePath));

            // 2. Создаём новый EXIF блок с нашими данными
            ...
            // 4. Вставляем новый EXIF блок перед EOI
    ...
    // Извлечение данных (ищет MakerNote в любом EXIF блоке)
    public (bool success, string message, string data) ExtractData(string imagePath)
    {
        try
        {
            ...
            // Ищем все EXIF блоки (может быть несколько)
    ...
    // Удаляет все EXIF сегменты (APP1 с "Exif")
            ...
            while (i < data.Length - 4)
            {
                if (data[i] == 0xFF && data[i + 1] == 0xE1)
                {
                        ...
                        data[i + 8] == 0x00 && data[i + 9] == 0x00)
                    {
                        ...
                        i += len + 4; // Пропускаем весь EXIF блок
                        continue;
                    }
                }
                ms.WriteByte(data[i]);
